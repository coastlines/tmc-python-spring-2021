...
API_KEY = os.environ['VAN_PASSWORD'] #create a civis credential with any username and just the api key as the pw
SOURCE_TABLE = os.environ['SOURCE_TABLE']
ID_TYPE = os.environ['ID_TYPE']
AC_STR = os.getenv('AC')
AC_ACTION = os.getenv('AC_ACTION').lower()
SG_STR = os.getenv('SG')
SG_ACTION = os.getenv('SG_ACTION').lower()
LOG_TABLE = os.environ['LOG_TABLE']
TIMESTAMP = datetime.datetime.now() #timestamp to be used for logtable

# TO DO for v2
# check for new folks or if the most recently applied ac/sg action is the inverse of the present action
# switch action input from string input to Boolean
# update log to log actual contents of the error
# look into switching to upsert_person_json()
# standardize tab indents
...
def main():
    ...
    AC = AC_STR.replace(' ','').split(',') if AC_STR else []
    SG = SG_STR.replace(' ','').split(',') if SG_STR else []

    # setup logging
    if not rs.table_exists(LOG_TABLE): #if Log Table does not already exist, create it
        # standard sql
        sql = "(source_vanid int, dest_vanid int, date_created timestamp, ac_load varchar(100),ac_action varchar(10), sg_load varchar(100), sg_action varchar(10));"
    ...
    if source_tbl.num_rows > 0:
      ...
      for ppl in source_tbl:
        ...
        for a in AC:
            if a != 'None':
                logger.info(f"Working on Activist Code: {a}...")
                acs = van.get_activist_codes().select_rows(lambda row: str(row.activistCodeId) == a or row.name.lower() == a.lower())

                if acs.num_rows == 0:
                    raise Exception("ERROR: There is no Activist Code with that ID or Name!")
                else:
                    if AC_ACTION == 'apply':
                        logger.info(f"Applying activist code {a}...")
                        try:
        ...
        for s in SG:
            if s != 'None':
                logger.info(f"Working on Supporter Group: {s}...")
                sgs = van.get_supporter_groups().select_rows(lambda row: str(row.id) == s or row.name.lower() == s.lower())

                if sgs.num_rows == 0:
                    raise Exception("ERROR: There is no Supporter Group with that ID or Name!")
                else:
                    if SG_ACTION == 'add':
                        logger.info(f"Applying activist code {s}...")
                        try:
